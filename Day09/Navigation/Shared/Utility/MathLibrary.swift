...
extension float4x4 {
  ...
  init(position: float3, target: float3, up: float3) {
    let direction = normalize(target - position)
    let right = normalize(cross(up, direction))
    let up = cross(direction, right)

    let X = float4(right.x, up.x, direction.x, 0)
    let Y = float4(right.y, up.y, direction.y, 0)
    let Z = float4(right.z, up.z, direction.z, 0)
    let W = float4(-dot(right, position), -dot(up, position), -dot(direction, position), 1)
